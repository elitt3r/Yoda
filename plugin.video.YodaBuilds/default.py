...
base='sicarium.com'
ADDON=xbmcaddon.Addon(id='plugin.video.YodaBuilds')
dialog = xbmcgui.Dialog()    
VERSION = "1.0.0"
PATH = "Wizard By Yoda Builds"            

    
def CATEGORIES():
    link = OPEN_URL('http://Media.myshh.net/wizard/jwiz.txt').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("Yoda Builds","Downloading ",'', 'Please Wait')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETE", 'LISTEN UP FOLKS - the only way to get the new changes to stick is', 'to force close kodi. Click ok to force Kodi to close,', 'DO NOT use the quit/exit options in Kodi., If the Force close does not close for some reason please Restart Device or kill task manaully')
    killxbmc()
